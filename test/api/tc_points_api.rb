...
class PointsApiTestCase < ApiTestCaseHelper
  def test_post_points_with_correct_params_works
    post "/api/user/#{user_id}/points", :moneySpent => "5.0", :isRide => "true"
  ...
  def test_post_points_with_wrong_params_fails
    post "/api/user/#{user_id}/points", :moneySpent => "true", :isRide => "5.0"
    assert_equal last_response.status, 400
    assert !last_response.ok?
  end

  def test_post_points_with_unknown_user_fails
    post "/api/user/no_such_user/points", :moneySpent => "5.0", :isRide => "true"
  ...
  def test_get_points_with_unknown_user_fails
    get "/api/user/no_such_user/points", :moneySpent => "5.0", :isRide => "true"
    assert_equal last_response.status, 404
    assert !last_response.ok?
  end
end
